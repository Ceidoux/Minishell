...
static void	ft_toc_tablecpy(char ***dst_table, char ***src_table, int size)
{
	...
	{	
		...
		if (!(dst_table[size]))
			exit(EXIT_FAILURE);
		if (src_table[size][0])
			ft_strlcpy(dst_table[size][0], src_table[size][0], ft_strlen(src_table[size][0]) + 1);
		if (src_table[size][1])
			ft_strlcpy(dst_table[size][1], src_table[size][1], ft_strlen(src_table[size][0]) + 1);
		if (src_table[size][2])
			ft_strlcpy(dst_table[size][2], src_table[size][2], ft_strlen(src_table[size][0]) + 1);
	}
}
